...
class JdbcKlantRepository implements KlantRepository{
	...
	private final RowMapper<Klant> klantRowMapper = 
			(resultSet,rowNum) -> new Klant(resultSet.getLong("id"),resultSet.getString("familienaam"),resultSet.getString("voornaam"),
	...
	public List<Klant> findKlantenByLetters(String letters) {
		return template.query(SELECT_KLANTEN_BY_LETTERS,Collections.singletonMap("letters",'%'+letters+'%'),klantRowMapper);
	}
