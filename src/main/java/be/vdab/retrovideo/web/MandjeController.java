...
class MandjeController {
	...
	ModelAndView toonMandje() {
		...
		Totaalprijs totaalprijs = new Totaalprijs(filmsInMandje.stream().map(film -> film.getPrijs())
				.reduce(BigDecimal.ZERO, (vorigeSom, getal) -> vorigeSom.add(getal)));
		return new ModelAndView(VIEW).addObject(new MandjeForm()).addObject("filmsInMandje", filmsInMandje)
