...
class BestellingController {
	...
	ModelAndView voorBevestigen(long klantId) {
		return new ModelAndView(BEVESTIGEN_VIEW)
				.addObject("aantalFilms",mandje.getFilmIds().size())
				.addObject("klant",reserveringService.getKlant(klantId));
	}
	
	@GetMapping("rapport/{klantId}")
	ModelAndView naBevestigen(long klantId) {
		...
		return new ModelAndView(RAPPORT_VIEW)
				.addObject("nietGereserveerdeFilms",nietGereserveerdeFilms);
	}
}